...
public static class SimpleMonitorMessagePlanService
{
    ...
    public static string CreateChatGptPrompt()
        => "Create a GPTDeskTop Monitor Only JSON message plan. Return ONLY valid JSON, no markdown fences and no explanation. Use schemaVersion 1. Keep every delaySeconds and defaultDelaySeconds between 15 and 3600 seconds. Use the messages array in the exact execution order. Each message object must contain text and enabled, and may contain label and delaySeconds. Set loop=true to repeat the sequence forever after the final enabled message, or loop=false to run the enabled sequence once and stop. Do not add fields that are not shown in the sample JSON I provide.";
    ...
    public static string BuildPreview(SimpleMonitorMessagePlan plan)
    {
        ...
        builder.AppendLine($"Mode: {(plan.Loop ? "Loop" : "Run once")}");
        builder.AppendLine($"Default delay: {plan.DefaultDelaySeconds} sec");
        builder.AppendLine($"Messages: {plan.Messages.Count} total / {enabledCount} enabled");
        builder.AppendLine();
        ...
        for (var index = 0; index < plan.Messages.Count; index++)
        {
            ...
            builder.AppendLine($"{index + 1}. [{(step.Enabled ? "ON" : "OFF")}] {label} — delay {step.EffectiveDelaySeconds(plan.DefaultDelaySeconds)} sec");
