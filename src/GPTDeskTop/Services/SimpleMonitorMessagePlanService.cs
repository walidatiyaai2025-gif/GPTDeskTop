...
public static class SimpleMonitorMessagePlanService
{
    ...
    private const int LegacyMinDelaySeconds = 15;
    public const int MinimumDelaySeconds = 30;
    ...
    public static SimpleMonitorMessagePlan ParseAndValidate(string json)
    {
        ...
        if (plan.DefaultDelaySeconds is < LegacyMinDelaySeconds or > MaximumDelaySeconds)
            throw new InvalidDataException($"defaultDelaySeconds must be between {LegacyMinDelaySeconds} and {MaximumDelaySeconds}.");
        ...
        for (var index = 0; index < plan.Messages.Count; index++)
        {
            ...
            if (step.DelaySeconds is < LegacyMinDelaySeconds or > MaximumDelaySeconds)
                throw new InvalidDataException($"Message #{index + 1} delaySeconds must be between {LegacyMinDelaySeconds} and {MaximumDelaySeconds} when supplied.");
            ...
            normalized.Add(new SimpleMonitorMessageStep
            {
                ...
                DelaySeconds = step.DelaySeconds.HasValue
                    ? NormalizeRuntimeDelaySeconds(step.DelaySeconds.Value)
                    : null,
                Sent = step.Sent
            });
        }
        ...
        return new SimpleMonitorMessagePlan
        {
            ...
            Loop = plan.Loop,
            DefaultDelaySeconds = NormalizeRuntimeDelaySeconds(plan.DefaultDelaySeconds),
            Messages = normalized
        };
    }

    public static int NormalizeRuntimeDelaySeconds(int seconds)
        => Math.Clamp(seconds, MinimumDelaySeconds, MaximumDelaySeconds);
    ...
    public static string CreateSampleJson()
    {
        var sample = new SimpleMonitorMessagePlan
        {
            ...
            Loop = false,
            DefaultDelaySeconds = MinimumDelaySeconds,
            Messages =
            [
                new SimpleMonitorMessageStep
                {
                    ...
                    Enabled = true,
                    DelaySeconds = MinimumDelaySeconds,
                    Sent = false
                },
                new SimpleMonitorMessageStep
                {
