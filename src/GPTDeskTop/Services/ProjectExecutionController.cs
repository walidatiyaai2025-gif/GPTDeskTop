...
public sealed class ProjectExecutionController
{
    ...
    public async Task<ProjectExecutionResult> StartOrContinueAsync(
        ProjectState state,
        long monitorId,
        CancellationToken cancellationToken = default)
    {
        ...
        var tab = tabs.FirstOrDefault(x => string.Equals(x.Id, monitor.TabId, StringComparison.Ordinal)
                                         && ChatGptConversationIdentity.IsSame(x.Url, monitor.Url))
