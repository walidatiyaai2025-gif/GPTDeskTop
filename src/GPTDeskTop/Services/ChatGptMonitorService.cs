...
public sealed class ChatGptMonitorService
{
            ...
            while (await timer.WaitForNextTickAsync(cancellationToken))
            {
                try
                {
                    ...
                    if (monitor.ConversationRotationEnabled && IsConversationContextLimit(text))
                    {
                        ...
                        var oldTab = tab; var newTab = await _chrome.CreateNewChatTabAsync(cancellationToken); await WaitForChatReadyAsync(monitor.Id, newTab, cancellationToken); await ApplyModelRouteAsync(monitor, newTab, recovery: false, contextRotation: true, cancellationToken);
                        var handoffService = new ConversationHandoffService(_database); var handoffMessage = await handoffService.BuildAsync(monitor, text, oldTab, cancellationToken); var startMessage = string.IsNullOrWhiteSpace(handoffMessage) ? (string.IsNullOrWhiteSpace(monitor.NewChatStartMessage) ? "كمل" : monitor.NewChatStartMessage) : handoffMessage; var sent = await SendWhenReadyAsync(monitor.Id, newTab, startMessage, cancellationToken); if (!sent) throw new InvalidOperationException("New ChatGPT chat opened but the rotation handoff could not be sent after waiting for the composer.");
    ...
    { var deadline = DateTimeOffset.UtcNow.AddSeconds(30); Exception? last = null; while (DateTimeOffset.UtcNow < deadline) { cancellationToken.ThrowIfCancellationRequested(); try { var state = await _chrome.GetChatStateAsync(tab, cancellationToken); if (!state.IsGenerating) { Activity?.Invoke(monitorId, $"[{tab.Title}] New Chat is ready for input."); return; } } catch (Exception ex) when (IsTransientChromeException(ex)) { last = ex; } await Task.Delay(500, cancellationToken); } throw new TimeoutException($"New Chat was not ready for input within 30 seconds.{(last is null ? string.Empty : $" Last CDP error: {last.Message}")}"); }
    ...
    { var deadline = DateTimeOffset.UtcNow.AddSeconds(30); var attempt = 0; while (DateTimeOffset.UtcNow < deadline) { cancellationToken.ThrowIfCancellationRequested(); attempt++; try { if (await _chrome.SendChatMessageVerifiedAsync(tab, message, cancellationToken)) { Activity?.Invoke(monitorId, $"Verified message accepted on attempt {attempt}."); return true; } } catch (Exception ex) when (IsTransientChromeException(ex)) { Activity?.Invoke(monitorId, $"Verified composer send retry {attempt}: {ex.GetType().Name}."); } await Task.Delay(750, cancellationToken); } return false; }
