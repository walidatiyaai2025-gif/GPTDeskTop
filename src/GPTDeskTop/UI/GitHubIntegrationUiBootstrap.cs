...
/// <summary>
/// Exposes GitHub settings as a dedicated stable dialog from the main command menu.
/// GitHubIntegrationControl is intentionally NOT injected into an already-visible SettingsForm:
/// doing so mutates the live settings control tree and can trigger repeated layout/presentation
/// passes while the GitHub control is building/loading its children.
/// </summary>
internal static class GitHubIntegrationUiBootstrap
{
    ...
    internal static bool TryInstall(MainForm main)
    {
        ...
        if (database is null || applicationMenu is null)
            return false; // Compact command menu may not be installed yet; retry on next Idle.

        if (applicationMenu.DropDownItems
        ...
        // Keep the existing general Settings command first, then Git Settings immediately after it.
        var settingsIndex = applicationMenu.DropDownItems
            .Cast<ToolStripItem>()
            .Select((item, index) => new { item, index })
            .FirstOrDefault(x => x.item is ToolStripMenuItem menuItem
                                 && string.Equals(menuItem.Text, "Settings", StringComparison.OrdinalIgnoreCase))
            ?.index ?? -1;
    ...
    {
        if (string.Equals(root.Text, text, StringComparison.OrdinalIgnoreCase))
            return root;
    ...
    private static async Task ShowGitSettingsAsync(MainForm owner, LocalDatabase database)
    {
        ...
        var control = dialog.Controls
            .Cast<Control>()
            .SelectMany(DescendantsAndSelf)
            .OfType<GitHubIntegrationControl>()
            .First();
    ...
    private static Form BuildGitSettingsDialog(LocalDatabase database)
    {
        ...
        header.Controls.Add(FluentTheme.CreateMutedLabel(
            "Configure GitHub repositories, branches and integration behavior independently from Application Settings."), 0, 1);
