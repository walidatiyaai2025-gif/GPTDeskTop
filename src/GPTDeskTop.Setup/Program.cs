...
internal static class Program
{
    ...
    private static void Main(string[] args)
    {
        ...
        try
        {
            ...
            if (MessageBox.Show(
                    $"Install {AppName} v{Version}?\n\nThe application will be installed for the current Windows user.",
                    $"{AppName} Setup",
            ...
            Install();
            MessageBox.Show(
                $"{AppName} v{Version} was installed successfully.\n\nA desktop and Start Menu shortcut were created.",
                $"{AppName} Setup",
    ...
    private static void Install()
    {
        ...
        ExtractRequiredResource("Payload.GPTDeskTop.exe", Path.Combine(installDir, "GPTDeskTop.exe"), overwrite: true);
        ExtractOptionalResource("Payload.appsettings.json", Path.Combine(installDir, "appsettings.json"), overwrite: false);
        ExtractOptionalResource("Payload.Version.txt", Path.Combine(installDir, "Version.txt"), overwrite: true);
        ExtractOptionalResource("Payload.ReleaseNotes.txt", Path.Combine(installDir, "ReleaseNotes.txt"), overwrite: true);
        ...
        CreateShortcut(
            Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "GPTDeskTop.lnk"),
            Path.Combine(installDir, "GPTDeskTop.exe"),
            installDir);
    ...
    {
        if (MessageBox.Show(
                "Remove GPTDeskTop from this computer?\n\nSaved database files will be preserved unless you delete them manually.",
                "Uninstall GPTDeskTop",
                MessageBoxButtons.YesNo,
        ...
        using (var key = Registry.CurrentUser.OpenSubKey(@"Software\Microsoft\Windows\CurrentVersion\Uninstall", writable: true))
            key?.DeleteSubKeyTree("GPTDeskTop", throwOnMissingSubKey: false);
    ...
    private static void StopRunningApplication()
    {
        foreach (var process in Process.GetProcessesByName("GPTDeskTop"))
        {
            ...
            {
                // Setup continues; file replacement will report a useful error if still locked.
            }
        }
    }
    ...
    private static void ExtractOptionalResource(string resourceName, string destination, bool overwrite)
        => ExtractResource(resourceName, destination, overwrite);
    ...
    private static void CreateShortcut(string shortcutPath, string targetPath, string workingDirectory, string arguments = "")
    {
        var shellType = Type.GetTypeFromProgID("WScript.Shell")
            ?? throw new InvalidOperationException("Windows Script Host is not available; shortcut creation failed.");
    ...
    private static string GetInstallDirectory()
        => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Programs", "GPTDeskTop");
