...
## Current Objective
Operate GPTDeskTop as a persistent multi-tab ChatGPT monitor manager. The application enumerates open debuggable Chrome tabs, allows any number of tabs to be added as saved monitors, gives every monitor its own automatic reply and enabled state, runs each monitor independently and concurrently, notifies the operator from the Windows taskbar whenever ChatGPT produces a new response, and automatically recovers a monitored tab when ChatGPT returns an error response.
...
| NOT-003 | Provide in-app tray menu choices for notification duration (3/5/8/10/15/30 sec) and save immediately | UI / Backend | Medium | Done | `Services/TrayNotificationService.cs`, `Data/LocalDatabase.cs` |
...
- Balloon duration is selectable from the GPTDeskTop tray menu and is persisted in SQLite.
