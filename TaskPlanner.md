...
- **UX:** Fluent/WinUI-inspired WinForms styling, right-click context menus, green/red runtime lamp, Crash Count card and Monitor Count card. The compact Commands menu removes duplicated operator action rows, while `ExpandableWorkspaceLayout` is the single DPI-aware final height owner for Development Plan / Runtime Health compact mode so the reclaimed space remains available to Live Activity through resize and DPI changes.
...
## CI Validation
The current stable source baseline is `bb3aa231cb06be46fbded0c5bbaa7acc3e1a2f4d` (stable build ID `bb3aa231`). All eight required GitHub Actions workflows passed for that exact source commit. The stable publisher wrote the validated self-contained Windows x64 executable into `Last release/GPTDeskTop.exe`; `Last release/RELEASE.txt` records the same source, 8/8 validation and SHA-256 `f6b760530f00dcdc568626fbc603a883395d09f18b22f55ab5be168359693830`.
