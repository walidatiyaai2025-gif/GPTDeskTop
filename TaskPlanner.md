...
- **Chrome Lifecycle:** native `SW_HIDE` / `SW_SHOW` is the preferred owned-window visibility path so active CDP sessions are not churned; CDP minimize/normal remains a fallback only when no native handle can be resolved. Hidden-window QA continuously proves both page-state polling and the `GetTabsAsync()` target enumeration used by Open Conversations, and CHR-006 physically proves the owned Chrome window transitions visible -> hidden -> visible.
- **UX:** Fluent/WinUI-inspired WinForms styling, right-click context menus, green/red runtime lamp, Crash Count card and Monitor Count card. The compact Commands menu removes duplicated operator action rows; `ExpandableWorkspaceLayout` is the single DPI-aware final height owner for Development Plan / Runtime Health compact mode; and UI-021 adds a 56px single-line status header so the remaining fixed hero space is returned to Live Activity while all four live metrics remain visible.
...
## CI Validation
The current stable source baseline is `ef527fca101ec32db892b016bea1392391924812` (stable build ID `ef527fca`, version `2.0.0.0`). All eight required GitHub Actions workflows passed for that exact source commit. The stable publisher wrote the validated self-contained Windows x64 executable into `Last release/GPTDeskTop.exe`; `Last release/RELEASE.txt` records the same source, 8/8 validation and SHA-256 `6a10978e8fdbf77caf13e93681dfe2ec17787f90f4b15af9a153e6705523987e`.

CHR-006 / QA-011 is verified: the physical Windows/Chrome gate requires the production-owned native window to transition `visible -> hidden -> visible` while `GetTabsAsync()` target enumeration and chat-state polling remain healthy.
...
- Physical Chrome QA requires a real `visible -> hidden -> visible` native-window transition and CHR-006 / QA-011 is automated and green.
- In compact operator mode, Development Plan remains 58 logical px collapsed / 118 expanded and Runtime Health remains 58 collapsed / 140 expanded after resize and DPI events; non-compact controls retain their legacy heights.
- The compact Commands-menu experience does not maintain a competing height event loop; final physical heights are enforced by `ExpandableWorkspaceLayout`.
- The main dashboard header is a single visual line at 56 logical px, retains all four existing live metrics, and keeps the removed subtitle purpose text in accessibility descriptions/tooltips.
- Header and metric-chip compact geometry is reapplied from logical dimensions after DPI changes without changing metric bindings.
