...
- **Passive Long-Response Monitoring:** elapsed time is never a page-mutation trigger. A slow, unchanged, empty, thinking or streaming response remains a passive wait state for as long as ChatGPT needs. `NoResponseRefreshSeconds` is retained only as a legacy persisted configuration key for backward-compatible databases/backups; the monitor loop does not consume it for refresh/recovery. Recovery is driven by explicit current error UI/error response text, delivery timeout, conversation/context limit, or genuine process/runtime failures. Current error detection is scoped to visible structured error UI instead of scanning arbitrary historical conversation body text.
...
- **Delivery Timeout Recovery:** when ChatGPT explicitly reports a message-delivery timeout, save the timeout, create a new ChatGPT tab, send the recovery message, move the existing Monitor ID to the new tab, continue monitoring, and close the old timed-out tab.
...
| MON-007 | Make monitoring error-driven: wait indefinitely for slow/thinking/streaming responses, never refresh solely because time elapsed, and scope current-error detection to visible error UI | Browser / Backend | High | CI Pending | `ChatGptMonitorService.cs`, `ChromeDevToolsService.cs`, passive-wait QA/tests |
...
| QA-006 | Set the legacy no-response value to 30 seconds, keep a real tab generating beyond that threshold with zero elapsed-time refreshes, then surface an explicit current error on another tab and verify exactly one error-driven refresh | QA Engineer | High | Automated / MON-007 CI pending | `NoResponseWatchdogProcessProbe.cs`, `qa-no-response-watchdog.yml` |
...
| QA-010 | Source-contract regression: monitor loop contains no elapsed-time refresh trigger and Chrome state detection never scans arbitrary body history for current errors | QA / Backend | High | Automated / MON-007 CI pending | `ChatMonitorErrorDrivenWaitRegressionTests.cs` |
...
The previous per-tab 30-second stale-refresh gate is historical baseline behavior and is intentionally superseded by MON-007. The replacement gate keeps the legacy 30-second setting in the database but requires zero time-driven reloads, validates a 40-second generating response on the same page load, rejects historical body text as a current error signal, and separately requires one explicit-error-driven refresh. MON-007 remains CI Pending until the branch/PR workflows are observed green.
