...
function Replace-Exact([string]$text, [string]$old, [string]$new, [string]$label) {
    if (-not $text.Contains($old)) { throw "MainForm patch anchor not found: $label" }
...
'@
$source = Replace-Exact $source $old $new 'shutdown overlay field'
...
$new = @'
        ...
        UpdateChromeVisibilityButtons();
'@
$source = Replace-Exact $source $old $new 'shutdown overlay host'
...
'@
$source = Replace-Exact $source $old $new 'missing monitor tab recovery'
...
$new = @'
        ...
        _ = CompleteShutdownAsync();
'@
$source = Replace-Exact $source $old $new 'non-freezing close entry'
...
'@
$source = Replace-Exact $source $old $new 'shutdown pipeline'

Set-Content -Path $path -Value $source -Encoding utf8NoBOM -NoNewline

# Remove temporary patch machinery from the product branch after it has done its job.
Remove-Item '.github/workflows/agent-mainform-recovery-patch.yml' -ErrorAction SilentlyContinue
Remove-Item '.github/scripts/patch-mainform-recovery.ps1' -ErrorAction SilentlyContinue

git config user.name 'github-actions[bot]'
git config user.email '41898282+github-actions[bot]@users.noreply.github.com'
git add -A
git commit -m 'Recover missing monitor tabs and show shutdown progress'
