...
replace_once(
    ...
    "<GPTDeskTopVersion>2.0.34</GPTDeskTopVersion>",
    "<GPTDeskTopVersion>2.0.35</GPTDeskTopVersion>",
)

# Safety gate: slower baseline pacing plus durable micro-break state.
...
replace_once(
    ...
    {
        public static DurableState Empty { get; } = new(
            false,
            0,
            null,
            null,
...
# Runner: enforce 30s minimum and count only durable confirmed deliveries.
...
# UI cannot advertise/select a delay lower than the enforced safety floor.
...
# Existing version/pacing assertions.
for test in [
...
# Runtime + ordering regression for the durable pacing contract.
pacing_test = Path("tests/GPTDeskTop.RuntimeTests/SimpleMonitorPacingRegressionTests.cs")
pacing_test.write_text(r'''using System.Reflection;
...
public sealed class SimpleMonitorPacingRegressionTests
{
    ...
    public async Task TwentyFiveConfirmedDeliveriesArmADurableTwoMinuteMicroBreak()
    {
        ...
        var constructor = gateType.GetConstructors(BindingFlags.Instance | BindingFlags.NonPublic)
        ...
        var recordConfirmed = gateType.GetMethod("RecordConfirmedDeliveryAsync", BindingFlags.Instance | BindingFlags.NonPublic)
            ?? throw new InvalidOperationException("RecordConfirmedDeliveryAsync was not found.");

        for (var i = 0; i < 25; i++)
        {
            var task = (Task)(recordConfirmed.Invoke(gate, new object?[] { CancellationToken.None })
                ?? throw new InvalidOperationException("Confirmed-delivery pacing task was not returned."));
            await task;
        }
    ...
    [Fact]
    public void ConfirmedDeliveryIsCountedOnlyAfterDurableCheckpointAndBeforeNextProgressMutation()
    {
        ...
        Assert.True(pacing > checkpoint);
        Assert.True(sentProgress > pacing);
        Assert.Contains("Math.Clamp(defaultDelaySeconds, 30, 3600)", runner, StringComparison.Ordinal);
    }

    [Fact]
    public void PacingStateIsPersistedWithTheExistingSafetyState()
    {
