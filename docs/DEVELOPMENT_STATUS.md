...
## Current Focus
The v1.8.0 release-readiness baseline remains intact. Post-1.8 maintenance is extending operator UX, supportability, portability and monitor/recovery safety without changing the validated monitor/recovery/delivery architecture. The searchable Stored History Explorer, read-only Runtime Health / Connection Center, privacy-safe Support Diagnostics Bundle, portable Configuration Backup Export, transactional Configuration Backup Restore/Import, Stable Monitor Conversation Identity Guard, shared Runtime / Recovery Conversation Identity Boundary, and Non-Destructive Pending Crash-Recovery Retry are merged on `main`. No tag or GitHub release has been created by this work.
